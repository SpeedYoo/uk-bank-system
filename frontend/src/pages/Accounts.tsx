...
const Accounts = () => {
    ...
    return (
        ...
        <>
            <div className="flex-1 p-4 sm:p-5 md:p-6 text-white flex flex-col lg:flex-row gap-5 md:gap-6 w-full">
                
                {/* LEWA KOLUMNA: LISTA KONT */}
                <div className="flex w-full lg:w-64 shrink-0 flex-col gap-2">
                    <h2 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 px-1">My Accounts</h2>
                        ...
                        {accounts.map((acc) => {
                            ...
                            return (
                                <button key={acc.id} onClick={() => setSelectedAccount(acc)} 
                                    ...
                                    ${isSelected ? (isJunior ? 'bg-purple-500/10 border-purple-500/50' : 'bg-emerald-500/10 border-emerald-500/30') : 'bg-[#161B22] lg:bg-transparent hover:bg-white/5 border-gray-800 lg:border-transparent'}`}>
                                    
                                    <div className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center ${isSelected ? (isJunior ? 'bg-purple-500 text-white' : 'bg-[#00FF85] text-black') : 'bg-white/5 text-gray-400'}`}>
                ...
                <div className="flex-1 flex flex-col h-full pr-2 pb-6 w-full">
                    {selectedAccount ? (
                        <div className="flex flex-col h-full animate-fadeIn max-w-6xl w-full mx-auto lg:mx-0">
                            ...
                            {/* NAGŁÓWEK KONTA */}
                            <div className="bg-[#161B22] border border-gray-800 rounded-3xl p-5 sm:p-6 mb-4 md:mb-5 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 sm:gap-6 shadow-lg w-full">
                                
                                <div className="w-full flex-1 min-w-0">
                                    <h2 className="text-[10px] sm:text-xs font-bold text-gray-500 uppercase tracking-widest mb-1.5">
                                        {selectedAccount.account_type === 'JUNIOR' ? `${selectedAccount.owner_first_name}'s Account` : `${firstName}'s Account`}
                                    </h2>
                                    <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter text-white break-words w-full">
                                        {formatCurrency(selectedAccount.balance, selectedAccount.currency)}
                                    </h1>
                                </div>
                                
                                <div className="bg-[#0B0E14] border border-gray-800 rounded-2xl p-4 sm:p-5 w-full xl:w-[400px] shrink-0">
                                    <div className="space-y-2.5 w-full">
                                        ...
                                        <div className="flex justify-between items-center gap-4 border-b border-gray-800/50 pb-2">
                                            <span className="text-gray-500 font-bold text-[9px] sm:text-[10px] uppercase tracking-widest whitespace-nowrap">Sort Code</span>
                                        ...
                                        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-1 sm:gap-4 pt-0.5">
                                            <span className="text-gray-500 font-bold text-[9px] sm:text-[10px] uppercase tracking-widest whitespace-nowrap">IBAN</span>
                                            <span className="font-mono text-[9px] sm:text-[10px] text-gray-400 sm:text-right break-all">
